...
namespace MediaInfoLib_MSCS
{
    ...
    public class Form1 : System.Windows.Forms.Form
    {
        ...
        private void Form1_Load(object sender, System.EventArgs e)
        {
            ...
            ToDisplay += "\r\n\r\nOpen\r\n";
            MI.Open(@"F:\Reng\baiduYun_Download\God.Of.Gamblers.1989.720p.HDTV.2Audio.DD2.0.x264-th71@beAst\God.Of.Gamblers.1989.720p.HDTV.2Audio.DD2.0.x264-th71@beAst.mkv");
        ...
        String ExampleWithStream()
        {
            ...
            FileStream From = new FileStream("aa.mkv", FileMode.Open, FileAccess.Read);
        ...
        private void formatString(String s)
        {
            ...
            {
                if (v[i].Contains("FileSize"))
                {
                    String[] tv = v[i].Split(new String[] { ":" }, StringSplitOptions.RemoveEmptyEntries);
                    Console.WriteLine("FileSize = " + tv[1]);
                }
                if (v[i].Contains("Duration"))
                {
                    String[] tv = v[i].Split(new String[] { ":" }, StringSplitOptions.RemoveEmptyEntries);
                    Console.WriteLine("Duration = " + tv[1]);
                }
                if (v[i].Contains("BitRate"))
                {
                    String[] tv = v[i].Split(new String[] { ":" }, StringSplitOptions.RemoveEmptyEntries);
                    Console.WriteLine("BitRate = " + tv[1]);
                }
                if (v[i].Contains("Width"))
                {
                    String[] tv = v[i].Split(new String[] { ":" }, StringSplitOptions.RemoveEmptyEntries);
                    Console.WriteLine("Width = " + tv[1]);
                }
                if (v[i].Contains("Height"))
                {
                    String[] tv = v[i].Split(new String[] { ":" }, StringSplitOptions.RemoveEmptyEntries);
                    Console.WriteLine("Height = " + tv[1]);
                }
            }
        }
